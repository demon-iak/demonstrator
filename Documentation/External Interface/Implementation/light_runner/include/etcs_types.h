...
/// The EVC simulator has been developped according to SRS Class 1 v2.3.0.
/// @section Overview
...
#define MAX_NV_SET_NB        1024

#define SRS_CLASS1_V10       16 ///< SRS class 1, version 1.0
#define SRS_CLASS1_V11       17 ///< SRS class 1, version 1.1
#define SRS_CLASS1_V20       32 ///< SRS class 1, version 2.0

// =======================    Axle load speed profile    =======================

/// Maximum axle load speed limitation
#define MAX_AXLELOAD_LIMIT 32

/// Maximum axle load profile discontinuities
#define MAX_AXLELOAD_PROF 32

/// Enumeration for train data internationale category
/// @SRS 7.5.1.62
typedef enum eAxleLoadCategory
{
    AXLE_CAT_A    = 0,
    AXLE_CAT_HS17 = 1,
    AXLE_CAT_B1   = 2,
...
#define STR_AXLE_LOAD_CATEGORY( d ) \
    ( d ) == AXLE_CAT_A ? "A" \
    : ( d ) == AXLE_CAT_HS17 ? "HS17" \
    : ( d ) == AXLE_CAT_B1 ? "B1" \
    : ( d ) == AXLE_CAT_B2 ? "B2" \
    : ( d ) == AXLE_CAT_C2 ? "C2" \
    : ( d ) == AXLE_CAT_C3 ? "C3" \
    ...
    : ( d ) == AXLE_CAT_D3 ? "D3" \
    : ( d ) == AXLE_CAT_D4 ? "D4" \
    : ( d ) == AXLE_CAT_D4XL ? "D4XL" \
    : ( d ) == AXLE_CAT_E4 ? "E4" \
    : ( d ) == AXLE_CAT_E5 ? "E5" : "???"

/// Mass for A axle load category (kg)
#define AXLE_CAT_A_MASS 16000

/// Mass for HS17 axle load category (kg)
#define AXLE_CAT_HS17_MASS 17000

/// Mass for B1 axle load category (kg)
#define AXLE_CAT_B1_MASS 18000

/// Mass for B2 axle load category (kg)
#define AXLE_CAT_B2_MASS 18000

/// Mass for C2 axle load category (kg)
#define AXLE_CAT_C2_MASS 20000

/// Mass for C3 axle load category (kg)
#define AXLE_CAT_C3_MASS 20000

/// Mass for C4 axle load category (kg)
#define AXLE_CAT_C4_MASS 20000

/// Mass for D2 axle load category (kg)
#define AXLE_CAT_D2_MASS 22500

/// Mass for D3 axle load category (kg)
#define AXLE_CAT_D3_MASS 22500

/// Mass for D4 axle load category (kg)
#define AXLE_CAT_D4_MASS 22500

/// Mass for D4XL axle load category (kg)
#define AXLE_CAT_D4XL_MASS 22500

/// Mass for E4 axle load category (kg)
#define AXLE_CAT_E4_MASS 25000

/// Mass for E5 axle load category (kg)
#define AXLE_CAT_E5_MASS 25000

/// @SRS 6.6.3.4.5 [3d]: Conversion of axle load category to axle load mass (kg)
#define AXLE_CAT_TO_AXLE_MASS( d ) \
    ( d ) == AXLE_CAT_A ? AXLE_CAT_A_MASS \
    : ( d ) == AXLE_CAT_HS17 ? AXLE_CAT_HS17_MASS \
    : ( d ) == AXLE_CAT_B1 ? AXLE_CAT_B1_MASS \
    : ( d ) == AXLE_CAT_B2 ? AXLE_CAT_B2_MASS \
    : ( d ) == AXLE_CAT_C2 ? AXLE_CAT_C2_MASS \
    : ( d ) == AXLE_CAT_C3 ? AXLE_CAT_C3_MASS \
    : ( d ) == AXLE_CAT_C4 ? AXLE_CAT_C4_MASS \
    : ( d ) == AXLE_CAT_D2 ? AXLE_CAT_D2_MASS \
    : ( d ) == AXLE_CAT_D3 ? AXLE_CAT_D3_MASS \
    : ( d ) == AXLE_CAT_D4 ? AXLE_CAT_D4_MASS \
    : ( d ) == AXLE_CAT_D4XL ? AXLE_CAT_D4XL_MASS \
    : ( d ) == AXLE_CAT_E4 ? AXLE_CAT_E4_MASS \
    : ( d ) == AXLE_CAT_E5 ? AXLE_CAT_E5_MASS : 0

/// @SRS 6.6.3.2.3 [5]: Conversion of axle load mass (kg) to  axle load category
#define AXLE_MASS_TO_AXLE_CAT( d ) \
    ( d ) <= AXLE_CAT_A_MASS ? AXLE_CAT_A \
    : ( d ) <= AXLE_CAT_HS17_MASS ? AXLE_CAT_HS17 \
    : ( d ) <= AXLE_CAT_B1_MASS ? AXLE_CAT_B1 \
    : ( d ) <= AXLE_CAT_C2_MASS ? AXLE_CAT_C2 \
    : ( d ) <= AXLE_CAT_D2_MASS ? AXLE_CAT_D2 : AXLE_CAT_E4

/// Special value for track kilometre reference value indicating that no geographical position can be calculated after this reference location
#define STOP_KP_CALCULATION 16777215

/// Enumeration for type of route suitability
typedef enum eVoltage
{
    VOLT_UNAVAILABLE = 0, ///< Line not fitted with traction system
    VOLT_AC_25000,        ///< AC 25kV 50Hz
    VOLT_AC_15000,        ///< AC 15kV 16.7Hz
    VOLT_DC_3000,         ///< DC 3kV
    VOLT_DC_1500,         ///< DC 1.5kV
    VOLT_DC_600_750,      ///< DC 600/750V
    MAX_VOLTAGE_TYPE
} eVoltage;

/// Macro to get a string corresponding to a route suitability
#define STR_VOLTAGE( d ) \
    ( d ) == VOLT_UNAVAILABLE ? "Line with no traction system" \
    : ( d ) == VOLT_AC_25000 ? "AC 25kV 50Hz" \
    : ( d ) == VOLT_AC_15000 ? "AC 15kV 16.7Hz" \
    : ( d ) == VOLT_DC_3000 ? "DC 3kV" \
    : ( d ) == VOLT_DC_1500 ? "DC 1.5kV " \
    : ( d ) == VOLT_DC_600_750 ? "DC 600/750V" \
    : "???"

typedef enum eKDryEBCL
{
    KDRY_EBCL_50_IDX = 0,
    KDRY_EBCL_90_IDX,
    KDRY_EBCL_99_IDX,
    KDRY_EBCL_99_9_IDX,
    KDRY_EBCL_99_99_IDX,
    KDRY_EBCL_99_999_IDX,
    KDRY_EBCL_99_9999_IDX,
    KDRY_EBCL_99_99999_IDX,
    KDRY_EBCL_99_999999_IDX,
    KDRY_EBCL_99_9999999_IDX,
    NB_KDRY
} eKDryEBCL;

/// Macro to get the string corresponding to the direction/orientation
#define STR_KDRY_EBCL( d ) \
    ( d ) == KDRY_EBCL_50_IDX ? "50%" \
    : ( d ) == KDRY_EBCL_90_IDX ? "90%" \
    : ( d ) == KDRY_EBCL_99_IDX ? "99%" \
    : ( d ) == KDRY_EBCL_99_9_IDX ? "99.9%" \
    : ( d ) == KDRY_EBCL_99_99_IDX ? "99.99%" \
    : ( d ) == KDRY_EBCL_99_999_IDX ? "99.999%" \
    : ( d ) == KDRY_EBCL_99_9999_IDX ? "99.9999%" \
    : ( d ) == KDRY_EBCL_99_99999_IDX ? "99.99999%" \
    : ( d ) == KDRY_EBCL_99_999999_IDX ? "99.999999%" \
    : ( d ) == KDRY_EBCL_99_9999999_IDX ? "99.9999999%" \
    : "???"

/// Situation of station platform
typedef enum ePlatformPosition
{
    PLATFORM_LEFT      = 0,
    PLATFORM_RIGHT     = 1,
    PLATFORM_BOTH      = 2,
    PLATFORM_NONE      = 3,
    PLATFORM_P_UNKNOWN = 4
} ePlatformPosition;

/// Macro to get a string corresponding to a route suitability
#define STR_PLATFORM_POSITION( d ) \
    ( d ) == PLATFORM_LEFT ? "PLATFORM LEFT SIDE" \
    : ( d ) == PLATFORM_RIGHT ? "PLATFORM RIGHT SIDE" \
    : ( d ) == PLATFORM_BOTH ? "PLATFORM BOTH SIDE" \
    : ( d ) == PLATFORM_NONE ? "NO PLATFORM" \
    : "UNKNOWN PLATFORM SIDE"

/// Situation of station platform
typedef enum ePlatformElevation
{
    PLATFORM_200_MM    = 0,
    PLATFORM_300_MM    = 1,
    PLATFORM_550_MM    = 2,
    ...
    PLATFORM_960_MM    = 12,
    PLATFORM_1100_MM   = 13,
    PLATFORM_E_UNKNOWN = 15
} ePlatformElevation;

/// Macro to get a string corresponding to a route suitability
#define STR_PLATFORM_ELEVATION( d ) \
    ( d ) == PLATFORM_200_MM ? "200 mm" \
    : ( d ) == PLATFORM_300_MM ? "300 mm" \
    : ( d ) == PLATFORM_550_MM ? "550 mm" \
    : ( d ) == PLATFORM_580_MM ? "580 mm" \
    : ( d ) == PLATFORM_680_MM ? "680 mm" \
    : ( d ) == PLATFORM_685_MM ? "685 mm" \
    : ( d ) == PLATFORM_730_MM ? "730 mm" \
    : ( d ) == PLATFORM_760_MM ? "760 mm" \
    : ( d ) == PLATFORM_840_MM ? "840 mm" \
    : ( d ) == PLATFORM_900_MM ? "900 mm" \
    : ( d ) == PLATFORM_915_MM ? "915 mm" \
    : ( d ) == PLATFORM_920_MM ? "920 mm" \
    : ( d ) == PLATFORM_960_MM ? "960 mm" \
    : ( d ) == PLATFORM_1100_MM ? "1100 mm" \
    : "UNKNOWN PLATFORM ELEVATION"
...
typedef enum
{
    ...
    STM_COM_KEY,  ///< key for the message queue for STM communication
...
typedef struct SModuleAttribute
{
    ...
    bool      bSuspended;  ///< indicate that the module is suspended
...
typedef enum
{
    ...
    CTRL_STM_EB_APP,     ///< EB application required from STM (for speed controller)
    CTRL_STM_EB_REL,     ///< release of EB required from STM (for speed controller)
    CTRL_STM_SB_APP,     ///< SB application required from STM (for speed controller)
    CTRL_STM_SB_REL,     ///< release of SB required from STM (for speed controller)
    ...
    MOD_ETCS_LEVEL_STM_ENTERED,       ///< etcs level STM entered (for mode manager)
    ...
    MOD_SN_ACK,                       ///< ack of STM National by the driver (for mode manager)
    ...
    MOD_SN_RQST,                      ///< STM National request (level tr) (for mode manager)
    ...
    DISP_STMREQUEST,                  ///< STM display request (for DMI interface manager)
    DISP_STM_NTC_DATA_ENTRY,          ///< STM NTC Data Entry
    DISP_STM_SPECIFIC_NTC_DATA_ENTRY, ///< STM specific NTC data entry request
    ...
    RAD_SOM_CONNECT_RQST,           ///< Request radio session establisment (in SoM scope) (for radio interface)
    RAD_CONNECT_RQST,               ///< Request radio session establisment (for radio interface)
    ...
    RAD_NETWORK_REGISTRATION_ORDER, ///< network registration order from driver or trackside
    ...
    ODO_TRACK_CONDITION_STATION, ///< Track condition station information

    // application code for STM controller
    STM_DATA_VIEW,                    ///< data view requested (for STM controller)
    STM_NEW_NATIONAL_DATA,            ///< new national data received (for STM controller)
    STM_NEW_TRAIN_DATA,               ///< new train data received (for STM controller)
    STM_NEW_DRIVERID,                 ///< new driver id received (for STM controller)
    STM_NEW_LANGUAGE,                 ///< new language selected (for STM controller)
    STM_SPECIFIC_NTC_DATA_VALUE,      ///< STM data (for STM controller)
    STM_BUTTON_EVENT_REPORT,          ///< Button event report (for STM controller)
    STM_ACK_REPLY,                    ///< Acknowledgement reply (for STM controller)
    STM_AIRGAP_MESSAGE,               ///<
    STM_TEST_PROCEDURE_PERMISSION,    ///<
    STM_ACTIVE_DMI_CHANNEL,           ///<
    STM_ANTENNA_BTM_ID,               ///<
    STM_ETCS_BTM_STATUS_MESSAGE,      ///<
    STM_ODOMETER_PARAMETERS,          ///<
    STM_ODOMETER_MULTICAST,           ///<
    STM_TRAIN_INTERFACE_INPUT_STATUS, ///<
    STM_TRAIN_INTERFACE_CMD_CFG,      ///<
    STM_BRAKE_INTERFACE_EB_SB_AV,     ///<
    STM_RQST_NTC_DATA_ENTRY,          ///<
    STM_RQST_END_OF_NTC_DATA_ENTRY,   ///<
    STM_EB_SB_PARAM,                  ///<
    // Received by multiple threads
    MISC_TRAIN_LENGTH_INCREASED, ///< Reduce storage data in rear of the train because driver has changed train length
...
#define STR_APPCODE( d ) \
    ...
    : ( d ) == CTRL_STM_EB_APP ? "CTRL_STM_EB_APP" \
    : ( d ) == CTRL_STM_EB_REL ? "CTRL_STM_EB_REL" \
    : ( d ) == CTRL_STM_SB_APP ? "CTRL_STM_SB_APP" \
    : ( d ) == CTRL_STM_SB_REL ? "CTRL_STM_SB_REL" \
    ...
    : ( d ) == MOD_ETCS_LEVEL_STM_ENTERED ? "MOD_ETCS_LEVEL_STM_ENTERED" \
    ...
    : ( d ) == DISP_STMREQUEST ? "DISP_STMREQUEST" \
    ...
    : ( d ) == STM_DATA_VIEW ? "STM_DATA_VIEW" \
    : ( d ) == STM_NEW_NATIONAL_DATA ? "STM_NEW_NATIONAL_DATA" \
    : ( d ) == STM_NEW_TRAIN_DATA ? "STM_NEW_TRAIN_DATA" \
    : ( d ) == STM_NEW_DRIVERID ? "STM_NEW_DRIVERID" \
    : ( d ) == STM_NEW_LANGUAGE ? "STM_NEW_LANGUAGE" \
    : ( d ) == STM_SPECIFIC_NTC_DATA_VALUE ? "STM_SPECIFIC_NTC_DATA_VALUE" \
    : ( d ) == STM_BUTTON_EVENT_REPORT ? "STM_BUTTON_EVENT_REPORT" \
    : ( d ) == STM_ACK_REPLY ? "STM_ACK_REPLY" : "???"
...
typedef enum
{
    ...
    ADDR_STMCTRL_ID,    ///< STM controller
...
#define STR_MODULEID( d ) \
    ...
    : ( d ) == ADDR_STMCTRL_ID ? "ADDR_STMCTRL_ID" \
...
typedef enum
{
    ...
    SYS_INFO_ACK_SWTCH_SN,          ///< ack for switch to STM National mode
    SYS_INFO_ACK_SH,                ///< ack for shunting mode (from SB to SH)
    ...
    SYS_INFO_TRANSITION_LEVEL_STM,  ///< transition to level STM
    SYS_INFO_TRANSITION_LEVEL_1,    ///< transition to level 1
    ...
    SYS_INFO_ACK_ENTRY_STM,         ///< ack transition to level STM
    SYS_INFO_ACK_ENTRY_L1,          ///< ack transition to level 1
...
#define STR_SYSTEM_INFO( d ) \
    ...
    : ( d ) == SYS_INFO_ACK_SWTCH_SN ? "ack for switch to STM National mode" \
    ...
    : ( d ) == SYS_INFO_TRANSITION_LEVEL_STM ? "transition to level STM" \
    ...
    : ( d ) == SYS_INFO_ACK_ENTRY_STM ? "ack transition to level STM" \
...
#define STR_SYS_STATUS_MESG( d ) \
    ( d ) == SSM_BALISE_READ_ERROR ? "Balise read error" \
    : ( d ) == SSM_COM_ERROR ? "Communication error" \
    : ( d ) == SSM_ENTERING_FS ? "Entering FS" \
    : ( d ) == SSM_ENTERING_OS ? "Entering OS" \
    : ( d ) == SSM_RUNAWAY_MOVEMENT ? "Runaway movement" \
    : ( d ) == SSM_SH_REFUSED ? "SH refused" \
    : ( d ) == SSM_TKS_NOT_COMPATIBLE ? "Trackside not compatible" \
    : ( d ) == SSM_TRAIN_DATA_CHANGED ? "Train data changed" \
    : ( d ) == SSM_TRAIN_REJECTED ? "Train is rejected" \
    : ( d ) == SSM_UNAUTH_PASS_EOA_LOA ? "Unauthorized passing of EOA / LOA" \
    : ( d ) == SSM_NO_MA_RECV_AT_LTR ? "No MA received at level transition" \
    : ( d ) == SSM_SR_DIST_EXCEED ? "SR distance exceeded" \
    : ( d ) == SSM_SH_STOP_ORDER ? "SH stop order" \
    : ( d ) == SSM_SR_STOP_ORDER ? "SR stop order" \
    : ( d ) == SSM_EMERGENCY_STOP ? "Emergency stop" \
    : ( d ) == SSM_TKS_MALFUNCTION ? "Trackside malfunction" \
    : ( d ) == SSM_SH_REQUEST_FAILED ? "SH request failed" \
    : ( d ) == SSM_RV_DIST_EXCEEDING ? "RV distance exceeded" \
    : ( d ) == SSM_NO_TRACK_DESC ? "No track description" \
    : ( d ) == SSM_NTC_BRAKE_DEMAND ? "NTC brake demand" \
    : ( d ) == SSM_ROUTE_UN_AXLE_LOAD ? "Route unsuitable ? axle load category" \
    : ( d ) == SSM_ROUTE_UN_LOADING_GAUGE ? "Route unsuitable ? loading gauge" \
    : ( d ) == SSM_ROUTE_UN_TRACTION_SYS ? "Route unsuitable ? traction system" \
    : ( d ) == SSM_RADIO_REG_FAILED ? "Radio network registration failed" \
    : "???"
...
///< Maximum STM message size
#define MAX_JRU_STM_MESSAGE_SIZE 238
...
typedef enum eJRUMessageTypeID
{
    ...
    JRU_STM_INFORMATION_ID             = 14, ///< JRU stm information message
...
#define STR_JRU_MESSAGE( d ) \
    ...
    : ( d ) == JRU_STM_INFORMATION_ID ? "JRU_STM_INFORMATION_ID" \
...
/// Enumeration for JRU STM information - STM event type
typedef enum eJruStmEventType
{
    JRU_STM_EVT_DISCONNECTION = 0, ///< Disconnection
    JRU_STM_EVT_DISPLAY       = 1, ///< Display of system status message
    JRU_STM_EVT_PACKET        = 2  ///< Reception/sending of STM packet
} eJruStmEventType;

/// Enumeration for JRU STM information - Sender of disconnect request
typedef enum eJruStmDisconnectSender
{
    JRU_STM_EVT_DISCONNECT_STM         = 0, ///< Disconnect request sent from STM
    JRU_STM_EVT_DISCONNECT_STM_CONTROL = 1  ///< Disconnect request sent from STM Control Function
} eJruStmDisconnectSender;

/// Enumeration for JRU STM information - Type of diconnection
typedef enum eJruStmDisconnectionType
{
    JRU_STM_EVT_FINAL_DISCONNECTION    = 0, ///< Final disconnect
    JRU_STM_EVT_NONFINAL_DISCONNECTION = 1  ///< Non final disconnection
} eJruStmDisconnectionType;

/// Structure to store JRU STM information message data
typedef struct sJruStmInformationData
{
    int32_t                  lStmId;
    eJruStmEventType         StmEventType;
    eJruStmDisconnectSender  DisconnectSender;
    eJruStmDisconnectionType DisconnectionType;
    int32_t                  lDisconnectReason;
    uint8_t                  lSystemStatusMessages;
    int32_t                  lStmPacketId;
    int32_t                  lStmPacketLength;
    uint8_t                  uszStmPacket[ MAX_JRU_STM_MESSAGE_SIZE ];
} sJruStmInformationData;

/// Enumeration for STM_SYSTEM_STATUS_MESSAGE masks
typedef enum eJruStmSystemStatusMsgMask
{
    JRU_NTC_BRAKE_DEMAND = 0x1, ///< Bit 01 - NTC brake demand
    JRU_NTC_NEEDS_DATA   = 0x2, ///< Bit 02 - NTC needs data
    JRU_NTC_FAILED       = 0x4  ///< Bit 03 - NTC failed
} eJruStmSystemStatusMsgMask;
...
/// Maximum size of radio message (in bytes)
#define RADIO_MSG_MAX_LENGTH 1023

/// Maximum size of euroradio message: primitive + radio message (in bytes) See Subset-037 5.3.1.3
#define EURORADIO_MSG_MAX_LENGTH ( 9 + RADIO_MSG_MAX_LENGTH )
...
typedef struct SRIMData
{
    ...
    uint8_t uszRadioMsg[ RADIO_MSG_MAX_LENGTH ]; ///< Radio message (SRS binary format)
...
typedef struct SProfile
{
    ...
    bool       abTrainLengthDelay[ MAX_PROFILE_DISCONTINUITIES ]; ///< Array of indicator for train length delay
    bool       abIsSSPModify[ MAX_PROFILE_DISCONTINUITIES ];      ///< Array of Indicator for SSP yet modified
...
#define STR_MODE( d ) \
    ...
    : ( d ) == MODE_STM_NATIONAL ? "National System" \
...
typedef struct SLevel
{
    ...
    int32_t lSTMId; ///< STM identifier (only used if Level == LEVEL_STM)
} SLevel;

///< Maximum STM identifier value
#define MAX_STM_VALUE 256

///< Maximum number of levels (0, 1, 2, 3 or STMx)
#define MAX_LEVEL_NB NB_LEVEL + MAX_STM_VALUE - 1

///< Maximum iterations in packets
#define MAX_ITERATIONS 32

///< Maximum size of STM product identifier
#define MAX_STM_PRODUCT_ID_SIZE 8

///< Enumeration for state of STM modules
typedef enum eSTMState
{
    POWER_OFF_STM_STATE      = 0, ///< STM power off status
    POWER_ON_STM_STATE       = 1, ///< STM power on status
    CONFIG_STM_STATE         = 2, ///< STM configuration status
    DATA_ENTRY_STM_STATE     = 3, ///< STM data entry status
    COLD_STANDBY_STM_STATE   = 4, ///< STM cold standby status
    HOT_STANDBY_STM_STATE    = 6, ///< STM hot standby status
    DATA_AVAILABLE_STM_STATE = 7, ///< STM data available status
    FAILURE_STM_STATE        = 8  ///< STM failure status
} eSTMState;

///< Structure used to store data about connected STM modules
typedef struct SSTMData
{
    bool      bAvailable; ///< Indication if STM module is available
    eSTMState STMState;   ///< STM module status
    bool      bIsolated;  ///< STM module isolated
    bool      bTrip;      ///< National trip procedure is currently active
} SSTMData;
...
typedef enum eLevelTr
{
    ...
    LTR_L0_STM,  ///< Level transition: L0  --> STM
    LTR_L0_L1,   ///< Level transition: L0  --> L1
    LTR_L0_L2,   ///< Level transition: L0  --> L2
    LTR_L0_L3,   ///< Level transition: L0  --> L3
    LTR_STM_L0,  ///< Level transition: STM --> L0
    LTR_STM_STM, ///< Level transition: STM --> STM
    LTR_STM_L1,  ///< Level transition: STM --> L1
    LTR_STM_L2,  ///< Level transition: STM --> L2
    LTR_STM_L3,  ///< Level transition: STM --> L3
    LTR_L1_L0,   ///< Level transition: L1  --> L0
    LTR_L1_STM,  ///< Level transition: L1  --> STM
    LTR_L1_L2,   ///< Level transition: L1  --> L2
    LTR_L1_L3,   ///< Level transition: L1  --> L3
    LTR_L2_L0,   ///< Level transition: L2  --> L0
    LTR_L2_STM,  ///< Level transition: L2  --> STM
    LTR_L2_L1,   ///< Level transition: L2  --> L1
    LTR_L2_L3,   ///< Level transition: L2  --> L3
    LTR_L3_L0,   ///< Level transition: L3  --> L0
    LTR_L3_STM,  ///< Level transition: L3  --> STM
    LTR_L3_L1,   ///< Level transition: L3  --> L1
...
typedef struct SLevelTransition
{
    ...
    int32_t  lFromSTMId;    ///< STM identifier of source STM      (only used if LevelTransition == LTR_STM_xxx)
    int32_t  lToSTMId;      ///< STM identifier of destination STM (only used if LevelTransition == LTR_xxx_STM)
...
typedef enum eEVCStatus
{
    ...
    OBS_STM_NATIONAL,                ///< EVC in SN
    OBS_REVERSING,                   ///< EVC in RV
...
#define STR_STATUS( d ) \
    ...
    : ( d ) == OBS_STM_NATIONAL ? "OBS_STM_NATIONAL" \
...
typedef struct SOnBoardStatus
{
    ...
    t_distance       dSTMTrLocation;    ///< Level STM transition border
...
typedef enum eFunctionType
{
    ...
    FCT_TYPE_REQ_MA_TAF,                           ///< Request MA on reception of "track ahead free up to the level 2/3 transition location" (only level 0,1,STM)
    ...
    FCT_TYPE_STM_MAX_SPEED,                        ///< STM max speed
    FCT_TYPE_STM_SYSTEM_SPEED,                     ///< STM system speed
    FCT_TYPE_LX_SPEED,                             ///< LX speed
    ...
    FCT_TYPE_ALLOW_ACCESS,                         ///< Allow access to MMI, train interfaces, JRU and odometer for STM
...
{
    DATA_TYPE_NATIONAL_VALUES,         ///< National Values
    DATA_TYPE_LINKING,                 ///< Linking
    ...
    DATA_TYPE_STM_MAX_SPEED,           ///< STM max speed
    DATA_TYPE_STM_SYSTEM,              ///< STM system speed/distance
    DATA_TYPE_LEVEL_TR,                ///< Level transition order
    ...
    DATA_TYPE_TKS_SYSTEM_VERSION,    ///< Trackside constituent System Version
...
#define STR_DATATYPE( d ) \
    ...
    : ( d ) == DATA_TYPE_STM_MAX_SPEED ? "DATA_TYPE_STM_MAX_SPEED" \
    : ( d ) == DATA_TYPE_STM_SYSTEM ? "DATA_TYPE_STM_SYSTEM" \
...
/// Enumeration for type of release speed
typedef enum eReleaseSpeedType
{
    RS_VALUE,   ///< use provided value
    RS_CALC,    ///< use value calculated on-board
    RS_NATIONAL ///< use national value for release speed
} eReleaseSpeedType;
...
typedef struct SMAData
{
    ...
    eReleaseSpeedType DangerPtReleaseSpeedType; ///< value, calculated, national value
    ...
    eReleaseSpeedType OverlaptReleaseSpeedType; ///< value, calculated, national value
...
typedef struct SExternalData
{
    ...
    int32_t    lSTMid;                     ///< STM identity (NID_STM)
...
typedef struct SFixedData
{
    ...
    t_distance dD_Metal; ///< Distance of metal immunity in Levels 0/STM
...
typedef struct SConfig
{
    ...
    SSTMData               aSTMAvailable[ MAX_STM_VALUE ];  ///< List of STMs available onboard (Index is STM identifier)
    ...
    SNationalData          uszAllNationalData[MAX_NV_SET_NB];   ///< Store all used national values set
    SNationalData          DelayNational;                       ///< National data to apply later
    ...
    int32_t                lSTMCommunicationPort; ///< Port (socket) number used for STM communication (between STM module and STM controller)
...
typedef struct SSupervisionData
{
    ...
    bool                 bMaxSTMSpeed;                  ///< Indicates if maximum speed is available for STM mode
    t_speed              dMaxSTMSpeed;                  ///< Maximum speed in STM mode until STM module switches to DA mode
...
typedef struct SBalStatic
{
    ...
    t_distance                                   dLastBaliseLocErr;     ///< Global variable containing the error on the last balise location
...
typedef struct SDAT_ManageNationalData
{
    ...
    SNationalData uszStaticAllNationalData[ MAX_NV_SET_NB ]; // store all national values sets
    bool          b1v1Applied;
    bool          b1v2Applied;
...
typedef struct SCTRL_ManageSTMBrakingRequest
{
    int32_t lEB_Required; ///< Requirement of EB , release at standstill & acknowledgement
    int32_t lSB_Required; ///< Requirement of SB , release if acknowledgement
} SCTRL_ManageSTMBrakingRequest;
...
typedef struct SCtrlStatic
{
    ...
    eTrainMode                    PreviousMode;
    eActiveCabin                  LastCabinStatus;
    bool                          bSpeedCtlInhibited;           ///< Inhibition of speed controller
    ...
    SCTRL_ManageSTMBrakingRequest Static_ManageSTMBrakingRequest;
